...
//        DIGSITE_BARGE(3724, 3808, 0),
    ...
    public static boolean handle(SpecialLocation specialLocation){
        switch (specialLocation){
            ...
            case BRIMHAVEN_DUNGEON:
                if (Ctx.ctx.clientLocalStorage.getVarpValueAt(393) != 1){
                    ...
                    if (clickObject(Filters.Objects.nameEquals("Dungeon entrance"), "Enter", () -> Ctx.getMyLocation().getY() > 4000 ?
            ...
            case RELLEKKA_TO_MISCELLANIA:
                final WalkerTile curr = Ctx.getMyLocation();
                if (NPCInteraction.clickNpc(Filters.NPCs.actionsEquals("Travel"), new String[]{"Travel"})){
                    WaitFor.condition(10000,() -> Ctx.getMyLocation().distanceTo(curr) > 20 ?
            ...
            case WATERBIRTH:
                String option = Ctx.ctx.npcs.getAll(Filters.NPCs.nameContains("Jarvald").combine(Filters.NPCs.actionsContains(
            ...
            case ZANARIS_RING:
                if (Ctx.ctx.equipment.getCount(772) == 0){
                ...
                if (InteractionHelper.click(
		                InteractionHelper.getRSObject(Filters.Objects.nameEquals("Door")), "Open", () -> SpecialLocation.ZANARIS_RING.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
                break;
            case LUMBRIDGE_ZANARIS_SHED:
                if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameEquals("Fairy ring")),
		                "Use", () -> LUMBRIDGE_ZANARIS_SHED.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
                break;
            ...
            case FINISHED_ROPE_TO_ROCK:
                if (RSItemHelper.use(954)){
                    ...
                    if (InteractionHelper.click(
		                    InteractionHelper.getRSObject(Filters.Objects.actionsContains("Swim to")), "Use Rope", () -> Ctx.getMyLocation().equals(Ctx.ctx.tiles.createWalkerTile(2513, 3468, 0)) ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                        return true;
                    }
                }
            ...
            case WATERFALL_DUNGEON_ENTRANCE:
                if (WATERFALL_DUNGEON.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 500){
                    return InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameEquals("Door")), "Open", () -> WATERFALL_DUNGEON_ENTRANCE.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE);
                } else if (RSItemHelper.use(954)){
                    if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameContains("Dead tree")), "Use Rope", () -> Ctx.getMyLocation().equals(Ctx.ctx.tiles.createWalkerTile(2511, 3463, 0)) ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                        return true;
                    }
                }
            ...
            case WATERFALL_DUNGEON:
                if (InteractionHelper.click(
		                InteractionHelper.getRSObject(Filters.Objects.idEquals(2010)), "Open", () -> Ctx.getMyLocation().getX() == WATERFALL_DUNGEON.x ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
                log.info("Failed to get to waterfall dungeon");
                break;
            case WATERFALL_FALL_DOWN:
                if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.actionsContains("Get in")), "Get in", () -> Ctx.getMyLocation().distanceTo(Ctx.ctx.tiles.createWalkerTile(2527, 3413, 0)) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
            ...
            case KALPHITE_TUNNEL:
                if (clickObject(Filters.Objects.nameEquals("Rope"), "Climb-up", () -> Ctx.getMyLocation().getY() < 9000 ?
                        WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)) {
                    return true;
                }
                break;
            case KALPHITE_TUNNEL_INSIDE:
                if (clickObject(Filters.Objects.nameEquals("Tunnel entrance").and(Filters.Objects.actionsEquals("Climb-down")), "Climb-down", () -> Ctx.getMyLocation().getY() > 4000 ?
                        WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                } else {
                    RSObject[] objects = new RSObject[] {Ctx.ctx.objects.findNearest(20,"Tunnel entrance")};
                    if (objects.length > 0 && walkToObject(objects[0])){
                        ...
                        if (actions != null && Arrays.stream(actions).noneMatch(s -> s.startsWith("Climb-down"))){
                            RSItem[] items = Ctx.ctx.inventory.getItems(954);
                ...
                break;
            case DWARF_CARTS_GE:
                RSObject[] objects = new RSObject[]{Ctx.ctx.objects.getNearest(Filters.Objects.nameEquals("Train cart").combine(new Filter<RSObject>() {
                    @Override
                    public boolean test(RSObject rsObject) {
                        return rsObject.getLocation().getY() == 10171;
                    }
                }, true))};
                if (clickObject(objects[0], "Ride", () -> Ctx.getMyLocation().getX() == specialLocation.x ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
            ...
            case BRIMHAVEN_DUNGEON_SURFACE:
                if (clickObject(Filters.Objects.nameEquals("Exit"), "Leave", () -> Ctx.getMyLocation().getY() < 8000 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                } else {
            ...
            case GNOME_EXIT:
                if (clickObject(Filters.Objects.nameEquals("Gate").combine(Filters.Objects.actionsContains("Open"),
                        ...
                        () -> {
                            ...
                            return Ctx.getMyLocation().getY() == 3383 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE;
                        })){
            ...
            case GNOME_SHORTCUT_ELKOY_EXIT:
                if (NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Elkoy"), new String[]{"Follow"})){
                    WalkerTile current = Ctx.getMyLocation();
                    if(WaitFor.condition(8000, () ->  Ctx.getMyLocation().distanceTo(current) > 20 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        return false;
                    }
            ...
            case GNOME_TREE_EXIT:
                if (clickObject(Filters.Objects.nameEquals("Tree Door").combine(Filters.Objects.actionsContains("Open"), true), "Open",
                        () -> Ctx.getMyLocation().getY() == 3492 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    WaitFor.milliseconds(1060, 1500);
                    return true;
                } else {
                    log.info("Could not navigate through gnome door.");
            ...
            case ZEAH_SAND_CRAB:
                if (InteractionHelper.click(InteractionHelper.getRSNPC(Filters.NPCs.nameEquals("Sandicrahb")), "Quick-travel") && WaitFor.condition(10000, () -> Ctx.getMyLocation().getY() >= 3457 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS){
            ...
            // case ARDY_LOG_WEST:
            // case ARDY_LOG_EAST:
            //     RSObject[] logSearch = new RSObject[] {Ctx.ctx.objects.getNearest(15, Filters.Objects.nameEquals("Log balance").combine(Filters.Objects.actionsContains("Walk-across"), true))};
            //     if (AccurateMouse.click(logSearch[0], "Walk-across")){
            //         int agilityXP = Ctx.ctx.skills.getCurrentExp(Ctx.ctx.skills.getIndex(Skill.AGILITY.toString()));
            //         if (WaitFor.condition(StdRandom.uniform(7600, 1200), () -> Ctx.ctx.skills.getCurrentExp(Ctx.ctx.skills.getIndex(Skill.AGILITY.toString())) > agilityXP ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS) {
            //             return true;
            //         }
            ...
            case GNOME_WAYDAR:
                if (NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Daero"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Ctx.getMyLocation().distanceTo(GNOME_WAYDAR.getWalkerTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
                    WaitFor.milliseconds(1000, 2000);
                    return true;
            ...
            case CRASH_ISLAND:
                if (NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Waydar"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Ctx.getMyLocation().distanceTo(CRASH_ISLAND.getWalkerTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
                    WaitFor.milliseconds(1000, 2000);
                    return true;
            ...
            case APE_ATOLL_GLIDER_CRASH:
                if (NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Lumdo"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Ctx.getMyLocation().distanceTo(APE_ATOLL_GLIDER_CRASH.getWalkerTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
                    WaitFor.milliseconds(1000, 2000);
                    return true;
            ...
            case HAM_OUTSIDE:
                if (clickObject(Filters.Objects.nameEquals("Ladder"), "Climb-up", () -> Ctx.getMyLocation().getY() < 4000 ?
                        WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)) {
                    return true;
                }
                break;
            ...
            case HAM_INSIDE:
                if (RSObjectHelper.exists(Filters.Objects.actionsContains("Pick-Lock"))){
                    ...
                    if (InteractionHelper.click(RSObjectHelper.get(Filters.Objects.actionsContains("Climb-down")), "Climb-down")){
                        WaitFor.condition(3000, () -> HAM_INSIDE.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE);
                        return true;
                    }
                }
                break;
            ...
            case FEROX_ENCLAVE_PORTAL_F2P:
                if(NPCInteraction.isConversationWindowUp() || clickObject(Filters.Objects.nameEquals("Large door"), "Open",() -> NPCInteraction.isConversationWindowUp() ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    ...
                    return WaitFor.condition(3000,
                            () -> FEROX_ENCLAVE_PORTAL_F2P.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
                }
                break;
            case CASTLE_WARS_DOOR:
                if(NPCInteraction.isConversationWindowUp() || clickObject(Filters.Objects.nameEquals("Castle Wars portal"), "Enter", () -> NPCInteraction.isConversationWindowUp() ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    NPCInteraction.handleConversationRegex("Yes");
                    return WaitFor.condition(3000,
                            () -> CASTLE_WARS_DOOR.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
                }
                break;
            ...
            case FOSSIL_ISLAND_BARGE:
                if(clickObject(Filters.Objects.nameEquals("Rowboat"),"Travel",() -> NPCInteraction.isConversationWindowUp() ?
                    ...
                    return WaitFor.condition(5000,
                            () -> FOSSIL_ISLAND_BARGE.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
                }
                break;
            ...
            case FOSSIL_ISLAND_FERRY_CAMP:
                if(Ctx.ctx.npcs.getNearest("Barge guard") != null){
                    if(NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Barge guard"),"Quick-Travel")){
                        System.out.println("Success");
                        return WaitFor.condition(8000,
                                () -> FOSSIL_ISLAND_FERRY_CAMP.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
                    }
                } else {
            ...
            case RELLEKKA_DOCK_FROM_ISLES:
                return NPCInteraction.clickNpc(Filters.NPCs.actionsEquals("Rellekka"),"Rellekka") &&
                        WaitFor.condition(15000,() -> RELLEKKA_DOCK_FROM_ISLES.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10
                                ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
            case JATIZSO_DOCK:
                return NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Mord Gunnars"),"Jatizso") &&
                        WaitFor.condition(15000,() -> JATIZSO_DOCK.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10
                                ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;
            case NEITIZNOT_DOCK:
                return NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Maria Gunnars"),"Neitiznot") &&
                        WaitFor.condition(15000,() -> NEITIZNOT_DOCK.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 10
                                ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS;

            case OBSERVATORY_INSIDE:
                return clickObject(Filters.Objects.nameEquals("Rope"),"Climb", () -> OBSERVATORY_INSIDE.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 5
            ...
            case OBSERVATORY_OUTSIDE:
                return (NPCInteraction.isConversationWindowUp() ||  clickObject(Filters.Objects.nameEquals("Door"),"Open",
                    ...
                    if(NPCInteraction.isConversationWindowUp())
                        NPCInteraction.handleConversation("Yes.");
                    return OBSERVATORY_OUTSIDE.getWalkerTile().distanceTo(Ctx.getMyLocation()) < 5
                            ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE;
                }) == WaitFor.Return.SUCCESS && WaitFor.milliseconds(600,1800) != null;

            case MOSS_GIANT_ISLAND_ROPE:
            case MOSS_GIANT_ISLAND_ROPE_LANDING:
                if(Ctx.getMyLocation().distanceTo(MOSS_GIANT_ISLAND_ROPE.getWalkerTile()) >= 2){
                    Ctx.ctx.walking.walkTo(MOSS_GIANT_ISLAND_ROPE.getWalkerTile());
                    WaitFor.milliseconds(200,400);
                }
                if (clickObject(Filters.Objects.nameEquals("Ropeswing"), "Swing-on", () -> Ctx.getMyLocation().getX() < 2708 ?
                        WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)) {
                    return true;
                }
                return false;
            ...
            case SHANTAY_PASS_EXIT:
                if(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()).getY() < 3117){
                    return clickObject(Filters.Objects.nameEquals("Shantay pass"),"Go-through", () -> SHANTAY_PASS_ENTRANCE.getWalkerTile().equals(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()))
                            ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) && WaitFor.milliseconds(600,1800) != null;
                } else if(Ctx.ctx.inventory.getCount(1854) == 0){
                    NPCInteraction.talkTo(Filters.NPCs.nameEquals("Shantay"),new String[]{"Buy-pass"}, new String[]{});
                }
                return Ctx.ctx.inventory.getCount(1854) > 0 && clickObject(Filters.Objects.nameEquals("Shantay pass"),"Go-through", () -> {
                    DoomsToggle.handleToggle();
                    return SHANTAY_PASS_EXIT.getWalkerTile().equals(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()))
                            ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE;
                }) && WaitFor.milliseconds(600,1800) != null;

            case PATERDOMUS_EAST_ENTRANCE:
                return clickObject(Filters.Objects.nameEquals("Trapdoor"), new String[]{"Open","Climb-down"}, () -> PATERDOMUS_EAST_ENTRANCE.getWalkerTile().equals(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()))
                    ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) && WaitFor.milliseconds(600,1800) != null;
            case PATERDOMUS_EAST_EXIT:
                return clickObject(Filters.Objects.nameEquals("Holy barrier"), "Pass-through", () -> PATERDOMUS_EAST_EXIT.getWalkerTile().equals(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()))
                    ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) && WaitFor.milliseconds(600,1800) != null;
        }
    ...
    public static boolean handleZeahBoats(String locationOption){
        ...
        RSNPC[] npcs = new RSNPC[]{Ctx.ctx.npcs.getNearest("Veos","Captain Magoro")};
        ...
        if(NPCInteraction.clickNpc(Filters.NPCs.nameEquals("Veos", "Captain Magoro"),new String[]{travelOption})){
            WalkerTile current = new WalkerTile(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()));
            if (WaitFor.condition(8000, () -> (ShipUtils.isOnShip() || new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()).distanceTo(current) > 20) ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS) {
                return false;
            }
    ...
    public static boolean handleKaramjaShip(){
        ...
        boolean pirateTreasureComplete = Ctx.ctx.clientLocalStorage.getVarpValueAt(71) >= 4;
    ...
    public static boolean walkToObject(RSObject object) {
        if (!object.isOnScreen() || !object.isClickable()){
            Ctx.ctx.walking.walkTo(object.getLocation());
    ...
    public static boolean clickObject(Predicate<RSObject> filter, String[] action, WaitFor.Condition condition){
        RSObject[] objects = new RSObject[]{Ctx.ctx.objects.getNearest(15, filter)};
    ...
    private static boolean handleFishingPlatform(){
        ...
        // RSNPC[] jeb = Ctx.ctx.npcs.getAll(Filters.NPCs.nameEquals("Jeb").and(Filters.NPCs.actionsEquals("Travel")));
        // if(jeb.length > 0){
        //     return InteractionHelper.click(jeb[0],"Travel") &&
        //             WaitFor.condition(20000, () -> Ctx.ctx.npcChat.getMessage() != null ?
        //                             WaitFor.Return.SUCCESS :
        //                             WaitFor.Return.IGNORE
    ...
    private static boolean takeFossilIslandBoat(String destination){
        if(NPCInteraction.isConversationWindowUp() || clickObject(
                ...
                () -> NPCInteraction.isConversationWindowUp() ?
                        WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
            WalkerTile myPos = new WalkerTile(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()));
            NPCInteraction.handleConversation(destination);
            return WaitFor.condition(5000,() -> new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()).distanceTo(myPos) > 10 ? WaitFor.Return.SUCCESS :
