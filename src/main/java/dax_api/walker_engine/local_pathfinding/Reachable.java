...
public class Reachable {
    ...
    private Reachable(WalkerTile homeTile) {
        map = generateMap(homeTile != null ? homeTile : new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()));
    }
    ...
    private static WalkerTile convertToLocal(int x, int y) {
        WalkerTile position = new WalkerTile(Ctx.ctx, x, y, new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()).getWorldLocation().getPlane(), x >= 104 || y >= 104 ? WalkerTile.TYPES.WORLD : WalkerTile.TYPES.SCENE);
    ...
    public static Reachable getMap() {
        return new Reachable(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()));
    }
