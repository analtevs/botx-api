...
public class DaxPathFinder {
    ...
    //     WalkerTile destination = new WalkerTile(MethodProvider.methods.walking.getDestination());
    //     if (destination == null) {
    ...
    //     return tiles.size() > 0 && !tiles.get(0).equals(new WalkerTile(MethodProvider.methods.players.getMyPlayer().getLocation())) ? tiles.get(0) : null;
    // }
    ...
    public static boolean canReach(Destination[][] map, WalkerTile tile) {
        if (tile.getPlane() != new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()).getWorldLocation().getPlane()) return false;
    ...
    public static Destination[][] getMap() {
        final WalkerTile home = new WalkerTile(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation())).toSceneTile();
    ...
    //         Point point1 = Calculations.convertRLPointToAWTPoint(Ctx.ctx.calc.tileToScreen(path.get(i), 0));
    //         Point point2 = Calculations.convertRLPointToAWTPoint(Ctx.ctx.calc.tileToScreen(previousTile, 0));
    ...
    //             Point point1 = Calculations.convertRLPointToAWTPoint(Ctx.ctx.calc.tileToScreen(tile, 0));
    //             Point point2 = Calculations.convertRLPointToAWTPoint(Ctx.ctx.calc.tileToScreen(parent, 0));
    ...
    //     return getWalkingHistory(Ctx.ctx.players.getMyPlayer());
    // }
